...
---------------------------------------------------------------
```{r}
summary_values_tbl <- reactive({processed_data_filtered_tbl() %>%
  summarize(health_metric = unique(order.id) %>% length(),
...
renderValueBox({
  valueBox(value = summary_values_tbl()$wise_metric, 
           caption = "Ration, Mountain to Road", 
           icon = "fa-brain", 
           color = "info")
})
...
shinyWidgets::radioGroupButtons(
  ...
  status    = "primary", 
  justified = TRUE, 
  checkIcon = list(yes = icon(name = "check", 
                              lib  = "glyphicon"),
                   no  = icon(name = "remove-circle", 
                              lib  = "glyphicon")))
```
...
output$plotly_2 <- renderPlotly({ g <- time_plot_tbl() %>%
